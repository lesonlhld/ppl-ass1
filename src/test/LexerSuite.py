...
class LexerSuite(unittest.TestCase):
    ...
    def test_wrong_token(self):
        self.assertTrue(TestLexer.checkLexeme("ab?svn","ab,Error Token ?",303))

    def test_if(self):
        self.assertTrue(TestLexer.checkLexeme("Else EndIf ElseIf","Else,EndIf,ElseIf,<EOF>",304))

    def test_illegal_escape_1(self):
        """test illegal escape"""
        self.assertTrue(TestLexer.checkLexeme(""" "a'bc\\"  ""","""Unclosed String: a'""",305))

    def test_array(self):
        """test array"""
        self.assertTrue(TestLexer.checkLexeme(""" {   1  ,  2,3  , 4}  ""","""{,3,,,2,,,3,,,4,},<EOF>""",306))
    ...
    def test_identifier8(self):
        self.assertTrue(TestLexer.checkLexeme("rdad 40oBhenK292aWfTSFLt6","rdad,40,oBhenK292aWfTSFLt6,<EOF>",308))
    def test_identifier9(self):
        self.assertTrue(TestLexer.checkLexeme("vJozdspl3p1iOcRiAI12 dUB 1.NM 2cY2","vJozdspl3p1iOcRiAI12,dUB,3.,Error Token N",309))
    def test_identifier10(self):
        self.assertTrue(TestLexer.checkLexeme("0gChhrlnd8xI1dsxd s dwdski6E","0,gChhrlnd8xI1dsxd,s,dwdski6E,<EOF>",310))


    def test_keyword1(self):
        self.assertTrue(TestLexer.checkLexeme("01anc mds For c brEaK 21mc continuE km","0,3,anc,mds,For,c,brEaK,21,mc,continuE,km,<EOF>",311))
    def test_keyword2(self):
        self.assertTrue(TestLexer.checkLexeme("tO doWnto Do if thEn elSE return1","tO,doWnto,Do,if,thEn,elSE,return1,<EOF>",312))
    def test_keyword3(self):
        self.assertTrue(TestLexer.checkLexeme("reTuRn whILe beGin eND function o","reTuRn,whILe,beGin,eND,function,o,<EOF>",313))
    def test_keyword4(self):
        self.assertTrue(TestLexer.checkLexeme("pROCEDURE TRUe 1.12VAR45 ARRay OF 12REAL","pROCEDURE,Error Token T",314))
    def test_keyword5(self):
        self.assertTrue(TestLexer.checkLexeme("BOOLEAN int 1.12INTEGER sTRIng not 12and","Error Token B",315))
    def test_keyword6(self):
        self.assertTrue(TestLexer.checkLexeme("oR diVModNTEGER Mod nottrEu","oR,diVModNTEGER,Error Token M",316))
    def test_keyword7(self):
        self.assertTrue(TestLexer.checkLexeme("If then else diMod 12String true ds false","If,then,else,diMod,32,Error Token S",317))
    def test_keyword8(self):
        self.assertTrue(TestLexer.checkLexeme("anD then elsediMod doWnTO.1trueds false","anD,then,elsediMod,doWnTO,.,3,trueds,false,<EOF>",318))
    def test_keyword9(self):
    ...
    def test_keyword10(self):
        self.assertTrue(TestLexer.checkLexeme("12while int 1.12 INTEGER oR 12function","12,while,int,3.12,Error Token I",320))

    def test_string1(self):
        self.assertTrue(TestLexer.checkLexeme(""" "This is a string containing tab \\t" ""","""This is a string containing tab \\t,<EOF>""",321))

    def test_operator1(self):
        self.assertTrue(TestLexer.checkLexeme("ddsls<l>02>=d1s<=123","ddsls,<,l,>,0,2,>=,d1s,<=,323,<EOF>",322))
    def test_operator2(self):
        self.assertTrue(TestLexer.checkLexeme("dlsd+1ds-*dmdsa/<>mdks","dlsd,+,3,ds,-,*,dmdsa,Error Token /",323))
    def test_operator3(self):
        self.assertTrue(TestLexer.checkLexeme("lsddl<>=1<>=112>=<=d1","lsddl,<,>=,3,<,>=,312,>=,<=,d1,<EOF>",324))
    def test_operator4(self):
        self.assertTrue(TestLexer.checkLexeme("13ek3<9e=9eend<>=Edasdndm<=>erE","13,ek3,<,9,e,=,9,eend,<,>=,Error Token E",325))
    def test_operator5(self):
        self.assertTrue(TestLexer.checkLexeme("djeiwjd1A<=>12>=<=d","djeiwjd1A,<=,>,32,>=,<=,d,<EOF>",326))
    def test_operator6(self):
        self.assertTrue(TestLexer.checkLexeme("<-mod>=not+mod+and+not","<,-,mod,>=,not,+,mod,+,and,+,not,<EOF>",327))
    def test_operator7(self):
        self.assertTrue(TestLexer.checkLexeme("*and<=>mod</<=","*,and,<=,>,mod,<,Error Token /",328))
    def test_operator8(self):
        self.assertTrue(TestLexer.checkLexeme("=or<=<><>=-<=>","=,or,<=,<,>,<,>=,-,<=,>,<EOF>",329))
    def test_operator9(self):
        self.assertTrue(TestLexer.checkLexeme("not<>=and>=mod<=-and","not,<,>=,and,>=,mod,<=,-,and,<EOF>",330))
    def test_operator10(self):
        self.assertTrue(TestLexer.checkLexeme("mod<=<===mod/<=<","mod,<=,<=,==,mod,Error Token /",331))


    def test_illegal_escape1(self):
        self.assertTrue(TestLexer.checkLexeme("\"bac\\ma\"","Illegal Escape In String: bac\\m",332))
    def test_illegal_escape2(self):
        self.assertTrue(TestLexer.checkLexeme("\"baMD\\HLSc\\na\"","Illegal Escape In String: baMD\\H",333))
    def test_illegal_escape3(self):
        self.assertTrue(TestLexer.checkLexeme("\",dls\\F12!LS\\kc\\na\"","Illegal Escape In String: ,dls\\F",334))
    def test_illegal_escape4(self):
        self.assertTrue(TestLexer.checkLexeme("\"ado\\mado\"","Illegal Escape In String: ado\\m",335))
    def test_illegal_escape5(self):
        self.assertTrue(TestLexer.checkLexeme("123abc \"xyz\k 456","123,abc,Illegal Escape In String: xyz\\k",336))
    def test_illegal_escape6(self):
        self.assertTrue(TestLexer.checkLexeme("\"aa\wb\"","Illegal Escape In String: aa\\w",337))
    def test_illegal_escape7(self):
        self.assertTrue(TestLexer.checkLexeme("ba+12+\"na\"\"md+1.2-468\lb","ba,+,32,+,na,Illegal Escape In String: md+1.2-468\\l",338))
    def test_illegal_escape8(self):
        self.assertTrue(TestLexer.checkLexeme("\"1.2+1.3+1.4\\o'\"123","Illegal Escape In String: 1.2+1.3+1.4\\o",339))
    def test_illegal_escape9(self):
        self.assertTrue(TestLexer.checkLexeme("**nac**+1.1 \"ba\\qm\f\"","+,3.1,Illegal Escape In String: ba\\q",340))
    def test_illegal_escape10(self):
        self.assertTrue(TestLexer.checkLexeme("\"concaheo\\uabc","Illegal Escape In String: concaheo\\u",341))

    def test_unclose_String1(self):
        self.assertTrue(TestLexer.checkLexeme("\"bacxyc","Unclosed String: bacxyc",342))
    def test_unclose_String2(self):
        self.assertTrue(TestLexer.checkLexeme("NmkobYn{!}+I1+\"`YS2h.J(","Error Token N",343))
    def test_unclose_String3(self):
        self.assertTrue(TestLexer.checkLexeme("\"acnv \" \"abc","acnv ,Unclosed String: abc",344))
    
    def test_unclose_String5(self):
        self.assertTrue(TestLexer.checkLexeme("a+11.2+\"mam.123\" 12 \"%^&","a,+,31.2,+,mam.123,32,Unclosed String: %^&",345))
    def test_unclose_String6(self):
        self.assertTrue(TestLexer.checkLexeme("38n\"[#Ffs?0ED\"0.\"T`#!7n","38,n,[#Ffs?0ED,0.,Unclosed String: T`#!7n",346))
    def test_unclose_String7(self):
        self.assertTrue(TestLexer.checkLexeme("\".Hub`22Y\"<{;}\"Y`=DxXhZKh",".Hub`22Y,<,{,;,},Unclosed String: Y`=DxXhZKh",347))
    def test_unclose_String8(self):
        self.assertTrue(TestLexer.checkLexeme("\"ULxM*`~.~+C_DISD2","Unclosed String: ULxM*`~.~+C_DISD2",348))
    def test_unclose_String9(self):
        self.assertTrue(TestLexer.checkLexeme("{SRs}\"Nk8U;\"rA\"@Y3*\"oV\"bh1","{,Error Token S",349))

    def test_integer_real1(self):
        self.assertTrue(TestLexer.checkLexeme("12.e5","12.e5,<EOF>",350))
    def test_integer_real2(self):
        self.assertTrue(TestLexer.checkLexeme("01","0,3,<EOF>",351))
    def test_integer_real3(self):
        #TODO: fix -42
        self.assertTrue(TestLexer.checkLexeme("Var x0.12e51.2","Var,x0,.,32e51,.,2,<EOF>",352))
    def test_integer_real4(self):
        self.assertTrue(TestLexer.checkLexeme(".",".,<EOF>",353))
    def test_integer_real5(self):
        #TODO:  fix -15
        self.assertTrue(TestLexer.checkLexeme("e--12 e12 E-15 99e 1 1. 1","e,-,-,32,e12,Error Token E",354))
    def test_integer_real6(self):
        self.assertTrue(TestLexer.checkLexeme("e-12.1 11.e11 12..12 2. .2 11e11 .1e-3","e,-,32.1,31.e11,32.,.,32,2.,.,2,31e11,.,3e-3,<EOF>",355))
    def test_integer_real7(self):
        self.assertTrue(TestLexer.checkLexeme("12.e0 -101 11.E 11.1e2","12.e0,-,301,31.,Error Token E",356))


    def test_comment1(self):
        self.assertTrue(TestLexer.checkLexeme("**12.e0 -101** 11.e 11.1e2","11.,e,31.1e2,<EOF>",357))
    def test_comment2(self):
        self.assertTrue(TestLexer.checkLexeme("**12.e0} -101** 11.1e2","11.1e2,<EOF>",358))
    def test_comment3(self):
        self.assertTrue(TestLexer.checkLexeme("{abc} 1.abc","{,abc,},3.,abc,<EOF>",359))
    def test_comment4(self):
        self.assertTrue(TestLexer.checkLexeme("**1.e0 - 101** {11.E} //22.12\\n","{,31.,Error Token E",360))
    def test_comment5(self):
        #TODO: Fix reallit , intlit
        self.assertTrue(TestLexer.checkLexeme("**12.e0\\nabc -101","Unterminated Comment",361))
    def test_comment6(self):
        self.assertTrue(TestLexer.checkLexeme("13ek3<9e=9eendE//dasd1.ndm<>d1.02erE","13,ek3,<,9,e,=,9,eendE,Error Token /",362))
    def test_comment7(self):
        self.assertTrue(TestLexer.checkLexeme("//dasd1.ndm\\n<>d1.02erE","Error Token /",363))
    def test_comment8(self):
        self.assertTrue(TestLexer.checkLexeme("{ +abc<>xyzb>cv } **12mds<>dsd=(*dsd*)*)**","{,+,abc,<,>,xyzb,>,cv,},<EOF>",364))
    def test_comment9(self):
        self.assertTrue(TestLexer.checkLexeme(""" * **""","*,Unterminated Comment",365))
    def test_comment10(self):
        self.assertTrue(TestLexer.checkLexeme("*** **","<EOF>",366))
    def test_comment11(self):
        self.assertTrue(TestLexer.checkLexeme("***** \"\"\"","*,Unclosed String: ",367))

    def test_comment12(self):
        self.assertTrue(TestLexer.checkLexeme("*****","*,<EOF>",368))
    def test_comment13(self):
        self.assertTrue(TestLexer.checkLexeme("*** **","<EOF>",369))
...
""" This is a test !@#$%^^&*()?/|, 1234567890 \\' ASD,abc,KJFLDSK '" \\f asdasd ,\\b\\f\\r\\n\\t\\'\\\\,<EOF>"""
    ...
    def test_05_string_contains_illegal_escape_3(self):
        ...
        output = """Illegal Escape In String:  This is a test '" asdsadasd \\ """
        self.assertTrue(TestLexer.checkLexeme(input,output,373))
    ...
    def test_21_lower_id(self):
        ...
        self.assertTrue(TestLexer.checkLexeme("abc","abc,<EOF>",378))

    def test_22_lower_upper_id(self):
        """test valid lower uppercase id"""
        self.assertTrue(TestLexer.checkLexeme("vaRiab","vaRiab,<EOF>",379))

    def test_23_lower_upper_digit_id(self):
        """test valid lower uppercase digit id"""
        self.assertTrue(TestLexer.checkLexeme("s0m3Th1ng123","s0m3Th1ng123,<EOF>",380))

    def test_24_lower_upper_digit_underscore_id(self):
        """test valid lower uppercase digit underscore id"""
        self.assertTrue(TestLexer.checkLexeme("s0m3Th1ng___123_Som3THiN6","s0m3Th1ng___123_Som3THiN6,<EOF>",381))

    def test_25_wrong_token(self):
        """test wrong token"""
        self.assertTrue(TestLexer.checkLexeme("ab?cde","ab,Error Token ?",382))

    def test_26_wrong_first_letter_upper_id(self):
        """test wrong id first upper letter of id"""
        self.assertTrue(TestLexer.checkLexeme("Wrong","Error Token W",383))
                      
    def test_27_wrong_first_letter_number(self):
        """test wrong id first number digit of id"""
        self.assertTrue(TestLexer.checkLexeme("9reat","9,reat,<EOF>",384))

    def test_28_wrong_first_letter_number(self):
        """test wrong id first letter underscore of id"""
        self.assertTrue(TestLexer.checkLexeme("_wrong","Error Token _",385))
    ...
    def test_31_valid_int_dec(self):
        ...
        self.assertTrue(TestLexer.checkLexeme("123 0","123,0,<EOF>",386))

    def test_32_valid_int_hex(self):
        """test valid hex integer"""
        self.assertTrue(TestLexer.checkLexeme("0xABCDEF12 0X100","0xABCDEF12,0X100,<EOF>",387))

    def test_33_valid_int_oct(self):
        """test valid oct integer"""
        self.assertTrue(TestLexer.checkLexeme("0o123 0O777","0o123,0O777,<EOF>",388))

    def test_34_invalid_int_dec_extra_0(self):
        """test invalid decimal integer with extra 0"""
        self.assertTrue(TestLexer.checkLexeme("0123","0,323,<EOF>",389))

    def test_35_invalid_int_hex_extra_0(self):
        """test invalid hex integer with extra 0"""
        self.assertTrue(TestLexer.checkLexeme("0x0123","0,x0123,<EOF>",390))

    def test_36_invalid_int_oct_extra_0(self):
        """test invalid hex integer with extra 0"""
        self.assertTrue(TestLexer.checkLexeme("0o0123","0,o0123,<EOF>",391))

    def test_37_invalid_int_prefix(self):
        """test invalid integer with wrong prefix"""
        self.assertTrue(TestLexer.checkLexeme("0a123","0,a123,<EOF>",392))

    def test_38_invalid_int_hex_letter(self):
        """test invalid hex integer with wrong letter"""
        self.assertTrue(TestLexer.checkLexeme("0X123abc","0X123,abc,<EOF>",393))

    def test_39_invalid_int_oct_letter(self):
        """test invalid hex integer with wrong letter"""
        self.assertTrue(TestLexer.checkLexeme("0o678","0o67,8,<EOF>",394))
    ...
    def test_45_float_dec_without_int_part(self):
        ...
        input = """.123"""
        output = """.,323,<EOF>"""
        self.assertTrue(TestLexer.checkLexeme(input,output,399))

    def test_46_float_exp_without_int_part(self):
        """test float"""
        input = """E123"""
        output = """Error Token E"""
        self.assertTrue(TestLexer.checkLexeme(input,output,401))

    def test_47_valid_bool(self):
        """test valid bool"""
        input = """TrueFalse"""
        output = """True,False,<EOF>"""
        self.assertTrue(TestLexer.checkLexeme(input,output,402))
...
"""Function: abc
    ...
    Endbody."""
        ...
        self.assertTrue(TestLexer.checkLexeme(input, expect,403))

    def test_integer(self):
        """test integers"""
        self.assertTrue(TestLexer.checkLexeme("Var x;","Var,x,;,<EOF>",404))

    def test_illegal_escape(self):
        """test illegal escape"""
        self.assertTrue(TestLexer.checkLexeme(""" "abc\\h def"  ""","""Illegal Escape In String: abc\\h""",405))
